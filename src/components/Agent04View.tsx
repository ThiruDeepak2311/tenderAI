...
export default function Agent04View() {
  ...
  return (
    <div className="space-y-6">
      {phase === "idle" && (
        <section className="rounded-xl border border-amber-300 bg-gradient-to-br from-amber-50 to-white p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-700">Ready to Build BOM</p>
          <h3 className="mt-2 text-xl font-bold text-slate-900">Run BOM &amp; Scope Agent</h3>
          <p className="mt-2 text-sm text-slate-700">Generates a category-grouped Bill of Materials with quantities, material specs, lead times, costs and supplier classifications. Surfaces scope exclusions and clarifications still needed from the client.</p>
          <button onClick={handleRun} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-amber-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-amber-700">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button onClick={handleAcceptDisclaimer} className="rounded-md bg-amber-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-amber-700">Continue</button>
            </div>
          </div>
        </div>
      )}
      ...
      {phase === "loading" && (
        <section className="rounded-xl border border-slate-200 bg-white p-8">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-amber-700">Agent Running</p>
          </div>
          <h3 className="mt-2 text-lg font-bold text-slate-900">Building Bill of Materials…</h3>
            ...
            {loadingSteps.map((step, idx) => {
              ...
              return (
                <li key={idx} className="flex items-center gap-3">
                  <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isDone ? "bg-amber-100" : isActive ? "bg-amber-100" : "bg-slate-100"}`}>
                    {isDone ? <svg className="h-3 w-3 text-amber-700" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : isActive ? <span className="h-2 w-2 animate-pulse rounded-full bg-amber-500" /> : <span className="h-1.5 w-1.5 rounded-full bg-slate-400" />}
                  </div>
        ...
        <>
          ...
          {c.categories.map((cat, idx) => (
            <section key={idx} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              <div className={`flex items-center justify-between border-b border-slate-200 px-5 py-3 ${categoryHeaderBg(cat.color)}`}>
                  ...
                  <tbody>
                    {cat.items.map((item, ii) => (
                      <tr key={ii} className="border-t border-slate-100">
                        <Td className="font-bold text-emerald-700">{item.tag}</Td>
                        <Td className="text-slate-800">{item.description}</Td>
                        <Td className="text-center text-slate-700">{item.qty} {item.unit}</Td>
                        <Td className="text-xs text-slate-600">{item.materialSpec}</Td>
                        <Td className={`text-center font-bold ${item.leadWeeks >= 40 ? "text-red-700" : item.leadWeeks >= 24 ? "text-amber-700" : "text-slate-700"}`}>{item.leadWeeks}</Td>
                        <Td className="text-right font-semibold text-emerald-700">{item.estimatedCost}</Td>
                        <Td>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Scope Exclusions ({c.scopeExclusions.length})</p>
          ...
          <section className="rounded-xl border border-amber-200 bg-amber-50 p-5">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-amber-700">⚠ Clarifications Needed from Client ({c.clarificationsNeeded.length})</p>
            <ul className="space-y-2">
...
function categoryHeaderBg(color: string) {
  return ({
    ...
    amber: "bg-amber-50",
    red: "bg-red-50",
  } as Record<string, string>)[color] || "bg-slate-50";
}
...
function supplierClassStyle(c: string) {
  if (c === "Single Source") return "bg-red-100 text-red-700";
  if (c === "OEM Only") return "bg-amber-100 text-amber-700";

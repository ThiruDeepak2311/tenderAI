...
export default function Agent01View() {
  ...
  const linkClass = "rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:border-emerald-500 hover:text-emerald-700";
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 font-semibold text-slate-900">{c.document.fileName}</p>
              <p className="mt-0.5 text-xs text-slate-500">{c.document.pageCount} pages · {c.document.fileSize}{phase === "done" ? ` · parsed ${c.document.parsedAt}` : " · awaiting processing"}</p>
            </div>
          </div>
      ...
      {phase === "idle" && (
        <section className="rounded-xl border border-emerald-300 bg-gradient-to-br from-emerald-50 to-white p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Ready to Process</p>
          <h3 className="mt-2 text-xl font-bold text-slate-900">Run RFI Intelligence Agent</h3>
          <p className="mt-2 text-sm text-slate-700">The agent will parse the tender document and extract structured intelligence — project metadata, key dates, scope, technical requirements, standards, evaluation criteria, compliance items, and risk flags.</p>
      ...
      {phase === "loading" && (
        <section className="rounded-xl border border-slate-200 bg-white p-8">
          ...
          <h3 className="mt-2 text-lg font-bold text-slate-900">Processing tender document…</h3>
              ...
              return (
                <li key={idx} className="flex items-center gap-3">
                  <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isDone ? "bg-emerald-100" : isActive ? "bg-amber-100" : "bg-slate-100"}`}>
                    {isDone ? (
                      <svg className="h-3 w-3 text-emerald-600" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                    ) : isActive ? (
          ...
          <Card label="Technical Requirements Extracted">
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {c.technicalRequirements.map((r, idx) => (
                <div key={idx} className="flex items-baseline justify-between gap-3 border-b border-slate-100 pb-2 last:border-b-0">
                  <p className="text-xs text-slate-500">{r.parameter}</p>
            ...
            <div className="space-y-2">
              {c.standardsDetected.map((s, idx) => (
                <div key={idx} className="flex items-start gap-3 rounded-md border border-slate-200 bg-slate-50 p-3">
                  <span className="shrink-0 rounded bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700">{s.body}</span>
                  <div className="min-w-0">
          ...
          <Card label="Evaluation Criteria & Weighting">
            <div className="space-y-2">
              {c.evaluationCriteria.map((e, idx) => (
                <div key={idx} className="flex items-center justify-between gap-4 rounded-md border border-slate-200 bg-slate-50 p-3">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900">{e.criterion}</p>
                    <p className="mt-0.5 text-xs text-slate-600">{e.detail}</p>
                  </div>
                  <span className="shrink-0 rounded-md bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700">{e.weight}</span>
                </div>
              ))}
            </div>
          </Card>
            ...
            <div className="space-y-3">
              {c.flaggedForReview.map((f, idx) => (
                <div key={idx} className="rounded-md border border-amber-200 bg-amber-50 p-3">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-slate-700">{f.detail}</p>
                </div>
              ))}
            </div>
          </Card>
...
function Card({ label, children, tone = "default" }: { label: string; children: React.ReactNode; tone?: "default" | "warning" }) {
  const borderClass = tone === "warning" ? "border-amber-300" : "border-slate-200";
  return (
    <section className={`rounded-xl border ${borderClass} bg-white p-5`}>
      <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-slate-500">{label}</p>
      {children}
    </section>
  );
}

    ...
    <div className="space-y-6">
      {phase === "idle" && (
        <section className="rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-500/5 to-slate-900 p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-emerald-400">Ready to Issue RFQs</p>
          <h3 className="mt-2 text-xl font-bold text-white">Run Supplier RFQ Agent</h3>
          <p className="mt-2 text-sm text-slate-300">Drafts RFQ packages per commodity, selects approved vendors, sets deadlines and assembles supporting documents. Outputs a tracking summary for the procurement team.</p>
      ...
      {phase === "loading" && (
        <section className="rounded-xl border border-slate-800 bg-slate-900 p-8">
          ...
          <h3 className="mt-2 text-lg font-bold text-white">Drafting supplier RFQs…</h3>
              ...
              return (
                <li key={idx} className="flex items-center gap-3">
                  <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isDone ? "bg-emerald-500/20" : isActive ? "bg-amber-500/20" : "bg-slate-800"}`}>
                    {isDone ? <svg className="h-3 w-3 text-emerald-400" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : isActive ? <span className="h-2 w-2 animate-pulse rounded-full bg-amber-400" /> : <span className="h-1.5 w-1.5 rounded-full bg-slate-600" />}
                  </div>
        ...
        <>
          ...
          {c.rfqPackages.map((pkg, idx) => (
            <section key={idx} className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
              <div className={`flex items-center justify-between border-b border-slate-800 px-5 py-3 ${priorityBg(pkg.priority)}`}>
                <div className="min-w-0">
                  <p className="text-[11px] text-slate-400">{pkg.rfqNo}</p>
                  <p className="mt-0.5 text-sm font-bold text-white">{pkg.commodity}</p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <span className="text-xs text-slate-400">Due: {pkg.responseDeadline}</span>
                </div>
              </div>
              <div className="space-y-3 p-5">
                <p className="text-sm leading-relaxed text-slate-300">{pkg.rfqScope}</p>

                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Vendors</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {pkg.vendors.map((v, vi) => (
                      <div key={vi} className="rounded-md border border-slate-800 bg-slate-900/50 px-3 py-2">
                        <p className="text-sm font-semibold text-white">{v.name}</p>
                        <p className="mt-0.5 text-[11px] text-slate-400">{v.country} · <span className="text-emerald-400">{v.status}</span></p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <ul className="mt-1.5 space-y-1">
                      {pkg.keyDocs.map((d, di) => (
                        <li key={di} className="flex items-center gap-2 text-xs text-slate-300">
                  ...
                  <div>
                    ...
                    <ul className="mt-1.5 space-y-1">
                      {pkg.commercialReqs.map((r, ri) => (
                        <li key={ri} className="flex items-start gap-2 text-xs text-slate-300">
...
function priorityBg(p: string) {
  if (p === "CRITICAL") return "bg-red-500/10";
  if (p === "HIGH") return "bg-amber-500/10";
  return "bg-slate-800/40";
}
function priorityPill(p: string) {
  if (p === "CRITICAL") return "bg-red-500/15 text-red-400";
  if (p === "HIGH") return "bg-amber-500/15 text-amber-400";

    ...
    <div className="space-y-6">
      {phase === "idle" && (
        <section className="rounded-xl border border-purple-300 bg-gradient-to-br from-purple-50 to-white p-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-wider text-purple-700">Ready to Search</p>
          <h3 className="mt-2 text-xl font-bold text-slate-900">Run Reference Design Agent</h3>
          <p className="mt-2 text-sm text-slate-700">Searches the project database for similar past projects, retrieves reusable drawings, BOMs and lessons learned, and maps the scope deltas vs the chosen reference.</p>
      ...
      {phase === "loading" && (
        <section className="rounded-xl border border-slate-200 bg-white p-8">
          ...
          <h3 className="mt-2 text-lg font-bold text-slate-900">Searching reference projects…</h3>
              ...
              return (
                <li key={idx} className="flex items-center gap-3">
                  <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isDone ? "bg-purple-100" : isActive ? "bg-amber-100" : "bg-slate-100"}`}>
                    {isDone ? <svg className="h-3 w-3 text-purple-600" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : isActive ? <span className="h-2 w-2 animate-pulse rounded-full bg-amber-500" /> : <span className="h-1.5 w-1.5 rounded-full bg-slate-400" />}
                  </div>
      ...
      {phase === "done" && (
        <>
          <section className="rounded-xl border-2 border-purple-400 bg-gradient-to-br from-purple-50 to-white p-6">
            <div className="flex items-start justify-between gap-6">
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-widest text-purple-700">Best Reference Match</p>
                <h2 className="mt-2 text-2xl font-extrabold text-slate-900">{c.bestMatch.project}</h2>
                <p className="mt-1 text-sm text-slate-700">{c.bestMatch.client} · {c.bestMatch.location} · {c.bestMatch.year}</p>
                <p className="mt-3 text-sm leading-relaxed text-slate-800">{c.bestMatch.scope}</p>
                <p className="mt-3 text-xs italic text-emerald-700">✓ {c.bestMatch.outcome}</p>
              </div>
              <div className="text-center">
                <div className="rounded-xl border-2 border-purple-500 bg-purple-100 px-5 py-3">
                  <p className="text-4xl font-extrabold text-purple-700">{c.bestMatch.similarity}<span className="text-lg">%</span></p>
                  <p className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">Similarity</p>
                </div>
                <p className="mt-2 text-xs text-slate-500">{c.bestMatch.contractValue} contract</p>
              </div>
            </div>
          </section>

          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Other Reference Projects Matched</p>
            <div className="space-y-2">
              {c.referenceProjects.map((p, idx) => (
                <div key={idx} className="flex items-start justify-between gap-4 rounded-md border border-slate-200 bg-slate-50 p-3">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900">{p.project}</p>
                    <p className="mt-0.5 text-xs text-slate-600">{p.client} · {p.location} · {p.year}</p>
                    <p className="mt-1 text-xs text-slate-700"><span className="text-slate-500">Δ </span>{p.keyDelta}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-lg font-bold text-purple-700">{p.similarity}%</p>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Reusable Engineering Documents</p>
            <div className="space-y-2">
              {c.reusableDocuments.map((d, idx) => (
                <div key={idx} className="flex items-start gap-3 rounded-md border border-slate-200 bg-slate-50 p-3">
                  <span className="shrink-0 rounded bg-purple-100 px-2 py-0.5 text-[10px] font-bold text-purple-700">{d.docType}</span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-slate-900">{d.docNo}</p>
                    <p className="mt-0.5 text-xs text-slate-600">{d.relevance}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="rounded-xl border border-amber-200 bg-amber-50 p-5">
            <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-amber-700">⚠ Scope Deltas — Adjustments Required</p>
            <div className="space-y-3">
              {c.scopeDeltas.map((d, idx) => (
                <div key={idx} className="rounded-md border border-amber-200 bg-white p-3">
                  <p className="text-sm font-semibold text-slate-900">{d.parameter}</p>
                  <p className="mt-1 text-xs text-slate-600">
                    <span className="text-slate-500">Reference:</span> {d.reference}
                    <span className="mx-2 text-amber-700">→</span>
                    <span className="text-slate-500">Required:</span> <span className="text-slate-900">{d.required}</span>
                  </p>
                  <p className="mt-1 text-xs text-amber-700">Action: {d.action}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-500">💡 Lessons Learned (from past project debriefs)</p>
          ...
          <section className="rounded-xl border border-purple-300 bg-gradient-to-br from-purple-50 to-white p-5">
            <div className="flex items-center gap-5">
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold uppercase tracking-wider text-purple-700">Engineering Reuse Estimate</p>
                <p className="mt-1 text-sm text-slate-700">Estimated portion of engineering scope reusable from past projects.</p>
                <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full rounded-full bg-purple-500" style={{ width: `${c.estimatedEngineeringReuse}%` }} />
                </div>
              </div>
              <p className="text-5xl font-extrabold text-purple-700">{c.estimatedEngineeringReuse}<span className="text-2xl text-slate-400">%</span></p>
            </div>
          </section>
